        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            with torch.cuda.amp.autocast():
                ...
                for ind in range(num_text_aug):
                    ...
                    if ind == 0:
                        logits_per_image, logits_per_text = create_logits(feature_dict[self.arg.model_args['head'][0]],
                                                                          text_embedding, logit_scale[:, 0].mean())

                        ground_truth = torch.tensor(label_g, dtype=feature_dict[self.arg.model_args['head'][0]].dtype,
                                                    device=device)
                    else:
